...
struct Select_Status_1x1_Provider: TimelineProvider {
		func placeholder(in context: Context) -> Select_Status_1x1_Entry {
			Select_Status_1x1_Entry(date: Date(), status: LoadingState.loading, items: [])
		}

		func getSnapshot(in context: Context, completion: @escaping (Select_Status_1x1_Entry) -> ()) {
			let entry = Select_Status_1x1_Entry(date: Date(), status: LoadingState.loading, items: [])
			completion(entry)
		}

		func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
				var entries: [Select_Status_1x1_Entry] = []
...
struct Select_Status_1x1_Entry: TimelineEntry {
	var date: Date
...
struct Select_Status_1x1_WidgetsEntryView : View {
	var entry: Select_Status_1x1_Provider.Entry
...
struct Select_Status_1x1_Widgets: Widget {
	let kind: String = "Widgets"

	var body: some WidgetConfiguration {
		StaticConfiguration(kind: kind, provider: Select_Status_1x1_Provider()) { entry in
...
#Preview(as: .systemSmall) {
	...
	Select_Status_1x1_Entry(date: Date.now, status: LoadingState.success, statusData: try! Data(contentsOf: URL(string: "https://github.githubassets.com/images/icons/emoji/unicode/1f334.png?v8")!), items: [
	...
	Select_Status_1x1_Entry(date: Date.now, status: LoadingState.loading, items: [])
	Select_Status_1x1_Entry(date: Date.now, status: LoadingState.failed, items: [])
}
